...
const council = [
  {
    ...
    title: 'Director, ATM Global Operations Research',
  ...
  },
  {
    ...
    title: 'DG, ATM Global Business Sch Entrepreneur Mgmt.',
    ...
    <section className="relative min-h-[70vh] bg-gradient-to-br from-blue-50 via-blue-100 to-blue-200 py-8 md:py-16 px-2 md:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-10 animate-fadeIn">
          <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 mb-4 tracking-tight drop-shadow-lg">Council & Advisory Board</h1>
          <p className="text-lg md:text-xl text-blue-900 font-medium max-w-2xl mx-auto">
            Meet our advisory, governing, and chair members who guide ATM Global Business School towards excellence and innovation in education.
          </p>
        </div>
        {/* Council Grid */}
          ...
          {council.map((member, idx) => (
            ...
            >
              <div className="relative mb-4">
                <img
                  src={import.meta.env.BASE_URL + 'src/assets/people/' + member.img}
                  alt={member.name}
              ...
              <h3 className={`text-lg font-semibold mb-1 transition-colors duration-300 ${activeIndex === idx ? 'text-blue-600' : 'text-blue-800'}`}>{member.name}</h3>
              ...
              <div className={`transition-all duration-300 overflow-hidden ${activeIndex === idx ? 'max-h-40 opacity-100 mt-4' : 'max-h-0 opacity-0 mt-0'}`}>
                <p className="text-blue-900 text-base mb-2">{member.bio}</p>
                <span className="block text-blue-500 italic font-medium">{member.remark}</span>
              </div>
            </div>
          ))}
        </div>
